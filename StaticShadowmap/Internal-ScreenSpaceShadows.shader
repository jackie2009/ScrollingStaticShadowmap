...
UNITY_DECLARE_SHADOWMAP(_shadowmap);
  ...
  #define UNITY_SAMPLE_SHADOW_extra(tex,coord)  UNITY_SAMPLE_SHADOW(tex,coord) //UNITY_SAMPLE_SHADOW(tex,coord) // (tex2D(tex,coord.xy).r>coord.z?1:0)
...
{
 // float4 _shadowmap_TexelSize=half4(1.0f/4096.0,1.0f/4096.0,4096.0,4096.0);
  
#if defined(SHADOWS_NATIVE)

	const float2 offset = float2(0.5,0.5);
	float2 uv = (coord.xy * _shadowmap_TexelSize.zw) + offset;
